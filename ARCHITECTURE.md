...
2. **Discovery Registry.** Doorzoekbaar op capability, sector, tier, certificeringsstatus.
   Elke agent publiceert een Agent Card v1.1 met vier datablokken (functional,
   architecture, guardrails, complianceCheck), integriteitsblok en risico-barometer 1–5.
...
- **API:** `v1/` bevat Next.js App Router-routes met Nederlandse domeinfouten en
  ...
  `@/lib/api`, `@/server/{auth,registry,escrow,audit}` — de domeinlaag van de
  ...
  `GET /v1/audit/logs/{tx}`), publieke check (`GET /v1/check/{id}`), oauth, sectors,
  capabilities, handboek, leads.
- **Schema's:** JSON Schema draft 2020-12, gevalideerd in de pipeline (manifest) en bij
  ingest (audit). Voorbeelden in `schemas/examples/` valideren in CI.
